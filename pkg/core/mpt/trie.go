...
func updateGeneration(h util.Uint256, gen uint32, st storage.Store) error {
	...
	case *BranchNode:
		for i := range n.Children {
			if !n.Children[i].(*HashNode).IsEmpty() {
				if err := updateGeneration(n.Children[i].Hash(), gen, st); err != nil {
					return err
				}
			}
		}
	case *ExtensionNode:
		if !n.next.(*HashNode).IsEmpty() {
			if err := updateGeneration(n.next.Hash(), gen, st); err != nil {
				return err
			}
		}
	case *HashNode:
...
func (t *Trie) PerformGC(root util.Uint256, st storage.Store) (int, error) {
	...
	gen := t.generation
	if err := updateGeneration(root, gen, st); err != nil {
		panic(err)
	}
	var n int
