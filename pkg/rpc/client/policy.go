...
func (c *Client) invokeNativePolicyMethod(operation string) (int64, error) {
	result, err := c.InvokeFunctionByScriptHash(PolicyContractHash, operation, []smartcontract.Parameter{}, nil)
	if err != nil {
		return 0, err
...
func (c *Client) GetBlockedAccounts() (native.BlockedAccounts, error) {
	result, err := c.InvokeFunctionByScriptHash(PolicyContractHash, "getBlockedAccounts", []smartcontract.Parameter{}, nil)
	if err != nil {
		return nil, err
	} else if result.State != "HALT" || len(result.Stack) == 0 {
